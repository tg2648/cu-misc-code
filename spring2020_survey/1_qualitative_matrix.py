...
FILE_NAME = 'A&S Spring 2020 Survey - Qualitative.docx'
...
for course_type in course_types:
    ...
    for question in questions.itertuples():
        ...
        responses = df[[matrix_value_col, text_entry_col]].sort_values(by=[matrix_value_col])
        for row in responses.itertuples(index=False):
            if isnotna(getattr(row, text_entry_col)):  # ignore blank responses
                p = doc.add_paragraph()
                r = p.add_run(f'{getattr(row, matrix_value_col)} - ')  # use getattr() to index the namedtuple with a variable
                r.italic = True
